...
export default async function BlogPostPage({ params }: Props) {
  ...
  const htmlContent = post.content
    ...
    .replace(/^### (.+)$/gm, '<h3 class="text-base font-semibold mt-6 mb-2" style="color:#1a1a1a">$1</h3>')
  ...
  return (
    <>
      ...
      <article className="mx-auto max-w-3xl px-4 py-20 sm:px-6">
        <Link href="/blog" className="mb-8 flex items-center gap-2 text-sm transition-colors hover:opacity-70" style={{ color: '#e87722' }}>
          <ArrowLeft className="h-4 w-4" aria-hidden="true" /> Bloga Dön
        </Link>

        <header className="mb-10">
          <div className="mb-4 flex items-center gap-3">
            <span className="rounded-full px-3 py-1 text-xs font-medium" style={{ backgroundColor: '#fff7f0', color: '#cc6419' }}>{post.category}</span>
            ...
            <time className="text-xs" style={{ color: '#8a7050' }} dateTime={post.date}>{post.date}</time>
          </div>
          <h1 className="text-3xl font-bold leading-tight sm:text-4xl" style={{ color: '#1a1a1a' }}>{post.title}</h1>
          <p className="mt-4 text-lg" style={{ color: '#5c5c5c' }}>{post.description}</p>
        </header>
        ...
        {related.length > 0 && (
          <section className="mt-16 border-t pt-12" style={{ borderColor: '#e0d4c0' }}>
            <h2 className="mb-6 text-lg font-bold" style={{ color: '#1a1a1a' }}>İlgili Makaleler</h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {related.map((r) => (
                <Link key={r.slug} href={`/blog/${r.slug}`} className="rounded-xl p-4 transition-shadow hover:shadow-sm" style={{ border: '1px solid #e0d4c0', backgroundColor: '#faf8f5' }}>
                  <p className="font-medium text-sm" style={{ color: '#1a1a1a' }}>{r.title}</p>
                  <p className="mt-1 text-xs" style={{ color: '#8a7050' }}>{r.readTime} dk</p>
                </Link>
              ))}
            </div>
          </section>
